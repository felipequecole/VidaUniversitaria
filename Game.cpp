...
Game::Game(){
    ...
    for (int i = 0; i < nINIMIGOS; i++) {
            ...
            Object[i].vivo = true; //ele ainda n�o morreu
            Object[i].evil = true;
    }
    ...
    for (int i = 0; i < nALIADOS; i++) {
            ...
            Aliados[i].vivo = true; //ele ainda n�o morreu
...
void Game::logic() { //logica de "lan�amento" dos inimigos
    bool isEvil;
    ...
    if (!pause){
        for (int i = 0; i <= nINIMIGOS; i++) { //ele "chama" o proximo objeto para tela
            if (Object[i].Sprite.getPosition().y < minDist && Object[i].naTela)    {
                    PodeSoltar = false;
                    //onScreen = (onScreen+1)%11; //se ele atinge essa posicao, outro come�a a cair
                    //contador++;
            }
        ...
        for (int i = 0; i <= nINIMIGOS; i++) {
                 //movement.reset();
                 //movement.resume();
            if (Object[i].naTela) { //s� move quem est� na tel
